...
void loop () {
  if (Serial.available()) {
    ...
    Serial.println (input);//print the letter saved in the input var
...
void shortspace () {delay(2*TB);}//space between letters
void space () {delay (4*TB);}//space between words

void dot () {digitalWrite(led,HIGH); delay (TB); digitalWrite(led,LOW); delay (TB);}//the dot this code make the led on for 300 than off for 300
void dash () {digitalWrite(led,HIGH); delay (3*TB); digitalWrite(led,LOW); delay (TB);}//the dash this code make the led on for 900 than off for 300
